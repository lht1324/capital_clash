...
function TerritoryDisplayEditModal({
    isOpen,
    onClose,
    ...
    const isProfileInfoChanged = useMemo(() => {
        ...
        const isWebsiteUrlChanged = profileData.websiteUrl !== userInvestorInfo?.website_url;
        ...
        return isNameChanged || isDescriptionChanged || isWebsiteUrlChanged || isContactEmailChanged || isAreaColorChanged;
    }, [profileData]);

    const isWebsiteUrlValid = useMemo(() => {
        if (!profileData.websiteUrl) return true;
        try {
            const url = new URL(profileData.websiteUrl);
    ...
    const onChangeWebsiteUrl = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    ...
    const handleSave = useCallback(async () => {
        // 입력값 유효성 검사
        if (profileData.websiteUrl && !isWebsiteUrlValid) {
    ...
    }, [isProfileInfoChanged, profileData, isWebsiteUrlValid, isContactEmailValid, userInvestorInfo, updateInvestor, onClose]);
    ...
    useEffect(() => {
        if (isOpen && userInvestorInfo) {
            ...
            setProfileData({
                ...
                websiteUrl: userInvestorInfo.website_url || '',
                contactEmail: userInvestorInfo.contact_email || '',
                areaColor: defaultColor
            });
        }
    }, [isOpen, userInvestorInfo, hexToHsl]);
                ...
                <div className="space-y-4 mb-6">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full p-3 border ${profileData.websiteUrl && !isWebsiteUrlValid ? 'border-red-500' : 'border-gray-300'} rounded-md shadow-sm focus:ring-2 ${profileData.websiteUrl && !isWebsiteUrlValid ? 'focus:ring-red-500 focus:border-red-500' : 'focus:ring-blue-500 focus:border-blue-500'} outline-none transition-all duration-200 bg-gray-50 hover:bg-white`}
                        />
                        {profileData.websiteUrl && !isWebsiteUrlValid && (
                            <p className="text-red-500 text-sm mt-1">The website URL format is not valid.<br/>(e.g. https://example.com)</p>
                        )}
                    </div>
                    <div>
...
export default memo(TerritoryDisplayEditModal);
